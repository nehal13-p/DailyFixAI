...
import { SyncState, _createAndReEmitRoom, defaultClientOpts, defaultSyncApiOpts } from "./sync.js";
...
class ExtensionToDevice {
  ...
  onResponse(data) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (events.length > 0 && _this2.cryptoCallbacks) {
      ...
      events.map(_this2.client.getEventMapper()).map(toDeviceEvent => {
        ...
        _this2.client.emit(ClientEvent.ToDeviceEvent, toDeviceEvent);
      });
  ...
  onResponse(data) {
    ...
    return _asyncToGenerator(function* () {
      if (data.global && data.global.length > 0) {
        _this3.processGlobalAccountData(data.global);
      }
      for (var roomId in data.rooms) {
        var accountDataEvents = mapEvents(_this3.client, roomId, data.rooms[roomId]);
        ...
        accountDataEvents.forEach(e => {
          _this3.client.emit(ClientEvent.Event, e);
        });
      }
    })();
  }
...
class ExtensionReceipts {
  ...
  onResponse(data) {
    ...
    return _asyncToGenerator(function* () {
      ...
      for (var roomId in data.rooms) {
        processEphemeralEvents(_this5.client, roomId, [data.rooms[roomId]]);
      }
    })();
  }
}
...
export class SlidingSyncSdk {
  ...
  onRoomData(roomId, roomData) {
    var _this6 = this;
    return _asyncToGenerator(function* () {
      var room = _this6.client.store.getRoom(roomId);
      if (!room) {
        if (!roomData.initial) {
          logger.debug("initial flag not set but no stored room exists for room ", roomId, roomData);
          return;
        }
        room = _createAndReEmitRoom(_this6.client, roomId, _this6.opts);
      }
      yield _this6.processRoomData(_this6.client, room, roomData);
    })();
  }
  ...
  processRoomData(client, room, roomData) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var stateEvents = mapEvents(_this7.client, room.roomId, roomData.required_state);
      // Prevent events from being decrypted ahead of time
      // this helps large account to speed up faster
      // room::decryptCriticalEvent is in charge of decrypting all the events
      // required for a client to function properly
      var timelineEvents = mapEvents(_this7.client, room.roomId, roomData.timeline, false);
      ...
      if (roomData.invite_state) {
        var inviteStateEvents = mapEvents(_this7.client, room.roomId, roomData.invite_state);
        yield _this7.injectRoomEvents(room, inviteStateEvents);
        if (roomData.initial) {
          room.recalculate();
          _this7.client.store.storeRoom(room);
          _this7.client.emit(ClientEvent.Room, room);
        }
        inviteStateEvents.forEach(e => {
          _this7.client.emit(ClientEvent.Event, e);
        });
      ...
      yield _this7.injectRoomEvents(room, stateEvents, timelineEvents, roomData.num_live);
      ...
      _this7.addNotifications(timelineEvents);
    ...
    return _asyncToGenerator(function* () {
      ...
      if (timelineWasEmpty) {
        ...
        for (var ev of stateEventList) {
          _this8.client.getPushActionsForEvent(ev);
        }
  ...
  sync() {
    ...
    return _asyncToGenerator(function* () {
      ...
      //      them from /sync.
      while (!_this9.client.isGuest()) {
        try {
          logger.debug("Getting push rules...");
          var result = yield _this9.client.getPushRules();
          logger.debug("Got push rules");
          _this9.client.pushRules = result;
          break;
        } catch (err) {
          logger.error("Getting push rules failed", err);
          if (_this9.shouldAbortSync(err)) {
            return;
          }
        }
      }
      ...
      // start syncing
      yield _this9.slidingSync.start();
    })();
  }
