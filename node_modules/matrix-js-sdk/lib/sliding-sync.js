...
import { sleep, defer } from "./utils.js";
...
export class SlidingSync extends TypedEventEmitter {
    ...
    return _asyncToGenerator(function* () {
      yield Promise.all(Object.keys(ext).map(/*#__PURE__*/function () {
        var _ref = _asyncToGenerator(function* (extName) {
          if (_this.extensions[extName].when() == ExtensionState.PreProcess) {
            yield _this.extensions[extName].onResponse(ext[extName]);
          }
        });
        return function (_x) {
    ...
    return _asyncToGenerator(function* () {
      yield Promise.all(Object.keys(ext).map(/*#__PURE__*/function () {
        var _ref2 = _asyncToGenerator(function* (extName) {
          if (_this2.extensions[extName].when() == ExtensionState.PostProcess) {
  ...
  invokeRoomDataListeners(roomId, roomData) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this3.emitPromised(SlidingSyncEvent.RoomData, roomId, roomData);
    })();
  }
      ...
      var _loop = function* _loop() {
          ...
          try {
            ...
            var reqBody = {
              lists: reqLists,
              pos: currentPos,
              timeout: _this4.timeoutMS,
              clientTimeout: _this4.timeoutMS + BUFFER_PERIOD_MS,
              extensions: _this4.getExtensionRequest(currentPos === undefined)
            };
            // check if we are (un)subscribing to a room and modify request this one time for it
            var newSubscriptions = difference(_this4.desiredRoomSubscriptions, _this4.confirmedRoomSubscriptions);
            var unsubscriptions = difference(_this4.confirmedRoomSubscriptions, _this4.desiredRoomSubscriptions);
            ...
            if (newSubscriptions.size > 0) {
              ...
              for (var roomId of newSubscriptions) {
                var customSubName = _this4.roomIdToCustomSubscription.get(roomId);
                var sub = _this4.roomSubscriptionInfo;
                if (customSubName && _this4.customSubscriptions.has(customSubName)) {
                  sub = _this4.customSubscriptions.get(customSubName);
                }
                reqBody.room_subscriptions[roomId] = sub;
              }
            }
            if (_this4.txnId) {
              reqBody.txn_id = _this4.txnId;
              _this4.txnId = null;
            }
            ...
            for (var _roomId of newSubscriptions) {
              _this4.confirmedRoomSubscriptions.add(_roomId);
            }
            for (var _roomId2 of unsubscriptions) {
              _this4.confirmedRoomSubscriptions.delete(_roomId2);
            }
            ...
            _this4.invokeLifecycleListeners(SlidingSyncState.RequestFinished, resp);
          } catch (err) {
            if (err.httpStatus) {
              _this4.invokeLifecycleListeners(SlidingSyncState.RequestFinished, null, err);
            ...
            } else if (_this4.needsResend || err.name === "AbortError") {
              return 0; // continue
          ...
          yield _this4.onPreExtensionsResponse(resp.extensions);
          for (var _roomId3 in resp.rooms) {
            yield _this4.invokeRoomDataListeners(_roomId3, resp.rooms[_roomId3]);
          }
